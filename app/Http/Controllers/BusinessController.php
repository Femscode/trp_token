...
class BusinessController extends Controller
{
    ...
    public function cdsprojects()
    {
        $data['projects'] = Project::latest()->paginate(10);

        return view('frontend.cdsprojects',$data);
       
    }
    public function fetchCDSProject(Request $request)
    {
        $data['projects'] = Project::where('year', $request->year)
        ->where('batch', $request->batch)
        ->where('cdsgroup', $request->cdsgroup)
        ->latest()->paginate(10);

        return view('frontend.cdsprojects',$data);
       
    }
    public function cdsproject($id)
    {
        $data['project'] = Project::find($id);
        $data['additionalimages'] = ProjectAdditionalImage::where('project_id',$id)->get();      

        return view('frontend.singleproject',$data);
